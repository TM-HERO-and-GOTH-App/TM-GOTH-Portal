...
function EditCaseDetail(props) {
    ...
    // Load when getCaseDetail updated
    useEffect(() => {
        onInitialLoad()
    }, [caseDetailData])
    ...
    const editCaseDetail = (e) => {
        ...
        CaseDetailService.updateCaseInfo(
            ...
            if (res.response === "FAILED") {
                return setAlert(true, false, 'The case failed to updated.')
            }
            if (res.status === 202) {
                return setAlert(true, false, `Missing field. (${res.data})`)
            }
    ...
    // Need to Pass relatedSrRowID data - FIX by using data from DB
    const ICPTT = () => {
    ...
    const NovaSR = () => {
        CreateCaseService.createNovaSR(
            ...
            caseDetailData?.CASE_CONTENT, userData.stakeholderName, caseDetailData?.CASE_CONTENT, 'EAI'
        ).then(res => {
            setCreateSiebelSRAndTT(true);
    ...
    const NovaTT = () => {
        CreateCaseService.createNovaTT(
            ...
            lovData.filter(filter => filter.L_ID === symptomCode).map(data => data.L_NAME)[0], symptomType,
            customerProfileFromNova.CustInfo.PrimaryContactRowID,
            customerProfileFromNova.CustInfo.PrimaryContactRowID,
                            ...
                            >
                                ...
                                <div className="profile-info-row">
                                    ...
                                    <div className="profile-info-value">
                                        <select
                                            className="chosen-select form-control"
                                            name="symptomType" id='symptomType'
                                            value={symptomCode}
                                            onChange={(e) => {
                                                setSymptomCode(e.target.value)
                                        ...
                                        >
                                            ...
                                            {
                                                siebelSystem === '0' || typeof siebelSystem === 'undefined' ?
                                    ...
                                    <div className="profile-info-value">
										<span className="editable" id="signup">
											<input
                                                ...
                                                type="text"
                                                name="srNum"
                                                placeholder='SR Number will populate automatically'
                                                value={srNumberInput}
