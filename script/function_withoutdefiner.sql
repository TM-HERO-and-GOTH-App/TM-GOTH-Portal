...
BEGIN
    ...
    FROM emdev.TBL_SDZ_STAFF T1
    LEFT OUTER JOIN emdev.TBL_HERO T2 ON T1.EMAIL = T2.EMAIL
    WHERE T1.EMAIL <> '' AND T2.EMAIL IS NULL;

	/* 2ND STEP */
	UPDATE emdev.TBL_HERO
	...
	FROM emdev.TBL_HERO T1
	LEFT OUTER JOIN emdev.TBL_HERO_PROFILE T2 ON T1.H_ID = T2.H_ID
	JOIN  emdev.TBL_SDZ_STAFF T3 ON T1.EMAIL = T3.EMAIL
...
BEGIN
    ...
    /*WHERE APP_ID = appID */
    WHERE APP_UPDATES = (SELECT MAX(APP_UPDATES) FROM emdev.TBL_APP_VERSION)
    AND APP_EXPIRED = 0
...
BEGIN
	...
	SELECT  T1.FULLNAME,
            ...
            T1.REGISTERED_DATE
	FROM emdev.TBL_HERO T1
	JOIN  emdev.TBL_HERO_PROFILE T2 ON T1.H_ID = T2.H_ID
	LEFT JOIN emdev.TBL_STAFF T3 ON T1.EMAIL = T3.EMAIL
	LEFT JOIN emdev.TBL_LOV T4 ON T2.SH_ID = T4.L_ID
	LEFT JOIN emdev.TBL_LOV T5 ON T2.STATE_ID = T5.L_ID
...
BEGIN
	...
	FROM emdev.TBL_CASE T1
		JOIN emdev.TBL_CASE_DETAIL T2 ON T1.C_ID = T2.C_ID
		JOIN emdev.TBL_HERO T3 ON T1.H_ID = T3.H_ID
...
REPLACE(REPLACE(REPLACE((SELECT E.CUSTOMER_NAME FROM  emdev.TBL_CUSTOMER_PROFILE E WHERE E.C_ID = A.C_ID),'\r',' '),'\n',' '),',',' ')CUST_NAME,
REPLACE(REPLACE(REPLACE(REPLACE(B.CASE_CONTENT, '\r', ' '), '\n', ' '),',',' '),';',' ')AS DESCRIPTION,
(SELECT D.FULLNAME FROM  emdev.TBL_HERO D WHERE D.H_ID = A.H_ID)HERO_LOGGER,
(SELECT D.EMAIL FROM  emdev.TBL_HERO D WHERE D.H_ID = A.H_ID)EMAIL_LOGGER,
(SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.AREA_LOCATION)AREA_LOCATION,
CASE
WHEN (SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.CASE_STATUS) = 'NEW' THEN 'UNASSIGNED'
ELSE (SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.CASE_STATUS)
END CASE_STATUS,
(SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.CASE_TYPE)CASE_TYPE,
CASE
WHEN (select E.FLAG from emdev.TBL_HERO D, emdev.TBL_STAFF E where D.H_ID = A.H_ID AND D.EMAIL = E.EMAIL) = 'VIP' THEN 'VIP'
ELSE NULL
END VIP,
REPLACE(REPLACE(B.PACKAGE_NAME,';',' '),',',' ') PACKAGE_NAME,(SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.PRODUCT_NAME)PRODUCT,
(SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.SEGMENT_ID)SEGMENT,
(SELECT D.FULLNAME FROM emdev.TBL_HERO D WHERE D.H_ID = A.OWNER_ID)CASE_OWNER,
(select F.L_NAME from emdev.TBL_HERO_PROFILE HP,emdev.TBL_LOV F where HP.H_ID = A.OWNER_ID and HP.SH_ID = F.L_ID) as GROUP_,
REPLACE(REPLACE(REPLACE(REPLACE(B.SR_NUM,'\r', ' '),'\n', ' '),';',' '),',',' ') SR_NUM,REPLACE(REPLACE(REPLACE(REPLACE(B.TT_NUM,'\r', ' '),'\n', ' '),';',' '),',',' ')TT_NUM,
CASE
WHEN B.CASE_STATUS = 70 THEN
(SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(G.REMARK,'\r', ' '),'\n', ' '), ',' ,' '),';',' '),'<',' ')FROM emdev.TBL_ACTION_REMARK G WHERE G.C_ID = A.C_ID AND G.REMARK_TYPE ='CLOSED' LIMIT 1)
END UPDATE_REMARKS,
CASE
WHEN B.CASE_STATUS = 70 THEN
(SELECT C.L_NAME FROM emdev.TBL_ACTION_REMARK G,emdev.TBL_LOV C WHERE G.C_ID = A.C_ID AND G.CT_ID = C.L_ID AND G.REMARK_TYPE ='CLOSED' LIMIT 1)
END CLOSURE_TYPE,
A.CREATED_DATE,DATE_FORMAT(A.CREATED_DATE,'%M %Y') MONTH_YEAR,
CASE
WHEN B.CASE_STATUS = 70 THEN
	CASE
    WHEN (SELECT COUNT(*) FROM emdev.TBL_ACTION_REMARK G WHERE G.C_ID = A.C_ID AND G.REMARK_TYPE ='IN-PROGRESS' LIMIT 1) > 0 THEN
    DATEDIFF(A.CLOSED_DATE,(SELECT G.LOGGED_DATE FROM emdev.TBL_ACTION_REMARK G WHERE G.C_ID = A.C_ID AND G.REMARK_TYPE ='IN-PROGRESS' LIMIT 1))
    ELSE
		CASE
        WHEN (SELECT COUNT(*) FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SUPPORT' LIMIT 1) > 0 THEN
			  DATEDIFF(A.CLOSED_DATE,(SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SUPPORT' LIMIT 1))
		ELSE  DATEDIFF(A.CLOSED_DATE,(SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SELF' LIMIT 1))
		END
	END
END AGING_CLOSED,
CASE
WHEN  B.CASE_STATUS = 70 THEN (SELECT G.LOGGED_DATE FROM emdev.TBL_ACTION_REMARK G WHERE G.C_ID = A.C_ID AND G.REMARK_TYPE ='CLOSED' LIMIT 1)
END CLOSED_DATE,
DATEDIFF((SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='TRANSFER' LIMIT 1),A.CREATED_DATE)AGING_UNASSIGNED ,
(SELECT H.LOGGED_DATE FROM emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='TRANSFER' LIMIT 1) DATE_UNASSIGNED ,
CASE
WHEN (SELECT COUNT(*) FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='TRANSFER' LIMIT 1) > 0 THEN
	CASE
    WHEN (SELECT COUNT(*) FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SUPPORT' LIMIT 1) > 0 THEN
		DATEDIFF((SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SUPPORT' LIMIT 1),(SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='TRANSFER' LIMIT 1))
	ELSE DATEDIFF((SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SELF' LIMIT 1),(SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='TRANSFER' LIMIT 1))
    END
ELSE
	CASE
	WHEN (SELECT COUNT(*) FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SUPPORT' LIMIT 1) > 0 THEN
		DATEDIFF((SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SUPPORT' LIMIT 1),A.CREATED_DATE)
	ELSE DATEDIFF((SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SELF' LIMIT 1),A.CREATED_DATE)
	END
END AGING_ASSIGNED,
CASE
WHEN (SELECT COUNT(*) FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SUPPORT' LIMIT 1) > 0 THEN
(SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SUPPORT' LIMIT 1)
ELSE (SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SELF' LIMIT 1)
END ASSIGNED_DATE,
CASE
WHEN (SELECT COUNT(*) FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SUPPORT' LIMIT 1) > 0 THEN
DATEDIFF((SELECT G.LOGGED_DATE FROM emdev.TBL_ACTION_REMARK G WHERE G.C_ID = A.C_ID AND G.REMARK_TYPE ='IN-PROGRESS' LIMIT 1),(SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SUPPORT' LIMIT 1))
ELSE DATEDIFF((SELECT G.LOGGED_DATE FROM emdev.TBL_ACTION_REMARK G WHERE G.C_ID = A.C_ID AND G.REMARK_TYPE ='IN-PROGRESS' LIMIT 1),(SELECT H.LOGGED_DATE FROM  emdev.TBL_ASSIGNMENT_LOG H WHERE H.C_ID = A.C_ID AND H.AL_TYPE ='SELF' LIMIT 1))
END AGING_INPROGRESS,
(SELECT G.LOGGED_DATE FROM emdev.TBL_ACTION_REMARK G WHERE G.C_ID = A.C_ID AND G.REMARK_TYPE ='IN-PROGRESS' LIMIT 1) INPROGRESS_DATE,
DATEDIFF((SELECT G.LOGGED_DATE FROM emdev.TBL_ACTION_REMARK G WHERE G.C_ID = A.C_ID AND G.REMARK_TYPE ='CANCELLED' LIMIT 1),A.CREATED_DATE) AGING_CANCELLED,
(SELECT G.LOGGED_DATE FROM emdev.TBL_ACTION_REMARK G WHERE G.C_ID = A.C_ID AND G.REMARK_TYPE ='CANCELLED' LIMIT 1) CANCELLED_DATE,
CASE
WHEN B.CASE_STATUS = 70 THEN DATEDIFF(A.CLOSED_DATE,A.CREATED_DATE)
WHEN B.CASE_STATUS = 73 THEN DATEDIFF((SELECT G.LOGGED_DATE FROM emdev.TBL_ACTION_REMARK G WHERE G.C_ID = A.C_ID AND G.REMARK_TYPE ='CANCELLED' LIMIT 1),A.CREATED_DATE)
...
BEGIN
	...
	SELECT  A.CASE_NUM as HERO_CASE_ID,
	REPLACE(REPLACE(REPLACE((SELECT UPPER(E.CUSTOMER_NAME) FROM  emdev.TBL_CUSTOMER_PROFILE E WHERE E.C_ID = A.C_ID),'\r',' '),'\n',' '),',',' ')CUST_NAME,
	REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(B.CASE_CONTENT, '\r', ' '), '\n', ' '),',',' '),';',' '),'<',' '),'\"',' ')AS DESCRIPTION,
	(SELECT UPPER(D.FULLNAME) FROM  emdev.TBL_HERO D WHERE D.H_ID = A.H_ID)HERO_LOGGER,
	(SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.AREA_LOCATION)AREA_LOCATION,
	CASE
	WHEN (SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.CASE_STATUS) = 'NEW' THEN 'UNASSIGNED'
	ELSE (SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.CASE_STATUS)
	...
	REPLACE(REPLACE(B.PACKAGE_NAME,';',' '),',',' ') PACKAGE_NAME,(SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.PRODUCT_NAME)PRODUCT,
	(SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.SEGMENT_ID)SEGMENT,
	(SELECT UPPER(D.FULLNAME) FROM emdev.TBL_HERO D WHERE D.H_ID = A.OWNER_ID)CASE_OWNER,
	/*(select F.L_NAME from emdev.TBL_HERO_PROFILE HP,emdev.TBL_LOV F where HP.H_ID = A.OWNER_ID and HP.SH_ID = F.L_ID) as GROUP_,*/
	(SELECT C.L_NAME FROM  emdev.TBL_LOV C WHERE C.L_ID = B.SH_ID) GROUP_,
	REPLACE(REPLACE(REPLACE(REPLACE(B.SR_NUM,'\r', ' '),'\n', ' '),';',' '),',',' ') SR_NUM,REPLACE(REPLACE(REPLACE(REPLACE(B.TT_NUM,'\r', ' '),'\n', ' '),';',' '),',',' ')TT_NUM,
	CASE
	WHEN B.CASE_STATUS = 70 THEN
	(SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(G.REMARK,'\r', ' '),'\n', ' '), ',' ,' '),';',' '),'<',' ')FROM emdev.TBL_ACTION_REMARK G WHERE G.C_ID = A.C_ID AND G.REMARK_TYPE ='CLOSED' LIMIT 1)
